 ...
 */
// TODO: figure out how tainted, and update
...
public static boolean captureLayout(IFigure root) {
	...
	// validate ---> contributeForLayout ---> [initial state] -> recordInitialState
	root.validate();
	Iterator iter = initialStates.keySet().iterator();
	if (!iter.hasNext()) {
...
public static boolean playbackState(Connection conn) {
	if (!PLAYBACK)
		return false;
    //double connProgress = progress;
    double connProgress = 1.0;
	...
	if (list1.size() == list2.size()) {
		...
		for (int i = 0; i < list1.size(); i++) {
			...
			pt1.x = (int)Math.round(pt1.x * connProgress + (1-connProgress) * pt2.x);
			pt1.y = (int)Math.round(pt1.y * connProgress + (1-connProgress) * pt2.y);
			points.addPoint(pt1);
		}
...
public static boolean playbackState(IFigure container) {
	...
	for (int i = 0; i < children.size(); i++) {
		...
		if (newBounds.getTopLeft().getDifference(child.getBounds().getTopLeft()).getArea() > 25)
			System.out.println("anim: " + child.getBounds() + " -->" + newBounds);
		*/
