 ...
 */
public class BackgroundableCompoundBorder
 ...
 * @since 2.0
 */
public BackgroundableCompoundBorder() { }
 ...
 * @since 2.0
 */
public BackgroundableCompoundBorder(Border outer, Border inner) {
	this.outer = outer;
	this.inner = inner;
}
...
/**
 * @see com.architexa.org.eclipse.draw2d.Border#getPreferredSize(IFigure)
 */
@Override
...
/**
 * @see com.architexa.org.eclipse.draw2d.Border#paint(IFigure, Graphics, Insets)
 */
@Override
...
public void paintBackground(IFigure figure, Graphics g, Insets insets) {
	if (outer != null && outer instanceof AbstractBackground) {
		g.pushState();
		((AbstractBackground)outer).paintBackground(figure, g, insets);
		g.popState();

		insets = insets.getAdded(outer.getInsets(figure));
	}
	if (inner != null && inner instanceof AbstractBackground)
		((AbstractBackground)inner).paintBackground(figure, g, insets);
}
