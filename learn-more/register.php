...
<?php include($_SERVER['DOCUMENT_ROOT'].'/includes/side-menu-header-empty.php') ?>

<?
	$plan = $_GET["plan"]; 
	if($plan=="")
		$plan = substr($_SERVER["REQUEST_URI"], 21);
?>
...
<? 
	/*
	 * if the registration page ends with a /local then we change the
	 * destination for the form submit
	 */
	if(preg_match("/.local$/", $plan)) { 
		$plan = preg_replace("/.local$/","",$plan)
?>
		url: "http://localhost:8888/users/createJSONP",
<? } else { ?>
...
<? } else { ?>
	<p>Sign up for an Architexa license.</p>
	<p>Architexa Team Edition Trial includes:
	<ul>
		<li>30 days free</li>
		<li>Powerful Diagramming Engine: Layered Architectural Diagrams, Class Diagrams, and Sequence Diagrams</li>
		<li>Share diagrams that are linked to your code</li>
...
<? } else { ?>
	<h2>Register for a Free Trial of the Team Edition</h2>
<? } ?>
</td><td></td></tr> 
...
<input type="hidden" style="width:225" name="planName" value="<?=$plan?>"/> 
<script type="text/javascript"> 
...
<input id="submitBtn" style="height:30px;font-size:20px;font-family:Calibri,Helvetica,Geneva,Arial,Verdana,sans-serif;" onclick="this.disabled=true;validate();" type="button" value="Proceed to Download" /></td></tr> 
</table>  
</FONT> 
</form> 
</div>
